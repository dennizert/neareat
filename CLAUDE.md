...
- **Premium/free tier enforcement:** `utils/premiumCheck.isPremiumUser(userId)` (active/trial subscription OR `ALWAYS_PREMIUM_EMAILS` allowlist) gates free limits across controllers; all gates return `403 { code: 'PREMIUM_REQUIRED' }`. Free limits — AI rec **1/day**, restaurant recommendation **1/day**, favorites **5**, collections **1**, reservations **lifetime 1**; restaurant menu + product photos hidden (`hasProductPhotos` flag). Restaurant free tier can't enable reservations, instant discount, campaign, or upload PRODUCT photos. Mobile catches the 403 and routes to a role-aware `PaywallScreen` (`utils/premiumGate.ts`).
  - **⚠️ BÜYÜK ELDEN GEÇİRME — planlandı, Sprint-18/19 (2026-06-23 kararı):** Bu model değişiyor. **(USER)** premium tamamen KALDIRILIYOR; özellikler **yıldız/seviye bazlı** açılacak (merkezi saf `utils/levelAccess.js`). 5 kademe yeni eşik (L1 0–49 · L2 50–99 · L3 100–149 · L4 150–249 · L5 250+): rezervasyon (L1 sadece ilk/onboarding · L2 1-ay · L3+ sınırsız + L3+ öncelik), AI/gün (1/5/10/20/∞), liste (L2+), favori (5/15/30/50/∞), restoran önerme (L3+). **Menü + ürün görselleri + ilk rezervasyon herkese açık** (gating kaldırılır). Puan farming önleme: doğrulanmış-ziyaret şartı + günlük tavan + referral sertleştirme. **6-aylık sezon sıfırlama** (`seasonStartAt`, herkes 2 seviye düşer taban L2). **(RESTAURANT)** premium kavramı kalkar → **tüm restoranlar tek-tip zorunlu ücretli**: 15 gün trial → **1.299,90 TL/ay** (`restaurant_premium` SKU); trial/abonelik bitince restoran pasifleşir. Detaylı kararlar memory `project_level_access_overhaul`.
