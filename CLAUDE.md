...
- **Restaurant capacity & occupancy (planlandı, Sprint-19):** `RestaurantProfile` koltuk kapasitesi alanı (mevcut `tableCount` = saat dilimi başına maks rezervasyon, AYRI kavram). **Koltuk + zaman-dilimi bazlı doluluk** (rezervasyon saati + ~2s çevrim süresi; çakışan dilimlerde koltuk düşülür; saf test-edilebilir çekirdek). Onayda restoran rezerve koltuk girer (varsayılan = `guestCount`); iptal/no-show/tamamlanan → koltuk geri açılır. Kullanıcıya **bant** gösterimi (Müsait/Az yer kaldı/Dolu, ham yüzde değil) + yetersiz kapasitede overbooking uyarısı (talep yine oluşur, restoran karar verir). L3+ talepleri panelde üstte + "Öncelikli Kullanıcı" rozeti.
