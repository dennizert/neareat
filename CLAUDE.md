...
- **Premium enforcement (RESTAURANT — hâlâ abonelik-bazlı, Sprint-19'da elden geçecek):** `utils/premiumCheck.isPremiumUser(userId)` restoran gate'lerini sürer (`restaurantAccountController`: rezervasyon kabulü açma, anlık indirim, kampanya, analitik, rapor, ürün foto yükleme) — hepsi `403 { code:'PREMIUM_REQUIRED' }`. Sprint-19: restoran premium kavramı kalkıp **tek-tip zorunlu ücretli** olacak (15 gün trial → **1.299,90 TL/ay** `restaurant_premium`; trial/abonelik bitince pasifleşme). Detaylı kararlar memory `project_level_access_overhaul`.
